...
def conversar_nuti(mensaje, historial):
    ...
    memoria_nuti.append({"role": "user", "content": mensaje})
    historial.append({"role": "user", "content": mensaje})

    respuesta = buscar_respuesta(mensaje, memoria_nuti)

    memoria_nuti.append({"role": "assistant", "content": respuesta})
...
def conversar_coach(mensaje, historial):
    ...
    memoria_coach.append({"role": "user", "content": mensaje})
    historial.append({"role": "user", "content": mensaje})

    respuesta = coach_emocional_inteligente(mensaje, memoria_coach)

    memoria_coach.append({"role": "assistant", "content": respuesta})
    historial.append({"role": "assistant", "content": respuesta})
...
def generar_plan_de_comida(tipo_comida):
    ...
    planes = {
        "Desayuno": {
            "base": "Avena con yogurt natural, fresas o plátano y una porción de nueces.",
            "bajar": "Huevos con verduras, una tortilla de maíz y fruta fresca.",
            "subir": "Huevos, pan integral, yogurt con fruta y un poco de crema de cacahuate.",
            "mantener": "Yogurt natural con fruta y avena, o huevos con pan integral.",
            "ligero": "Si tienes poca hambre: yogurt griego con fruta y semillas.",
        },
        "Comida": {
            "base": "Plato equilibrado con pollo, arroz o pasta, verduras y aguacate.",
            "bajar": "Proteína magra con ensalada grande, arroz moderado y agua natural.",
            "subir": "Pollo o carne con arroz, frijoles, aguacate y verduras.",
            "mantener": "Bowl con proteína, carbohidrato completo, verduras y grasa saludable.",
            "ligero": "Si quieres algo más ligero: ensalada completa con atún, pollo o huevo.",
        },
        "Cena": {
            "base": "Cena ligera con proteína, verduras y una porción pequeña de carbohidrato.",
            "bajar": "Ensalada grande con pollo, atún o huevo y una tostada horneada.",
            "subir": "Sándwich integral con proteína, verduras y yogurt o fruta.",
            "mantener": "Omelette con verduras o bowl pequeño con proteína y vegetales.",
    ...
    if "bajar" in meta:
        recomendacion = opciones["bajar"]
    elif "subir" in meta:
        recomendacion = opciones["subir"]
    elif "mantener" in meta:
        recomendacion = opciones["mantener"]
    else:
        recomendacion = opciones["base"]
    ...
    return (
        ...
        f"**Sugerencia:** {recomendacion}\n\n"
        f"{extra_texto}"
        f"**Meta para esta comida:** aproximadamente **{kcal_objetivo} kcal** y **{prot_objetivo} g de proteína**.\n\n"
        f"**Tu meta del día:** **{calorias_totales} kcal** y **{proteina_objetivo} g de proteína**.\n\n"
        f"**Te queda hoy:** **{calorias_restantes} kcal** y **{proteina_restante} g de proteína**.\n\n"
        f"**Progreso actual:** {progreso['detalle']} - {progreso['texto']}\n\n"
...
def analizar_comida_con_ia_avanzada(imagen):
    ...
    prompt = """
Analiza la foto de comida como nutriólogo visual.
Observa con cuidado: ingredientes principales, acompañamientos, toppings, salsas, crema, leche, azúcar, aceite, fritura, empanizado, bebidas y preparación.
Si parece postre, licuado, fruta con crema, smoothie, yogurt, helado o alimento congelado, dilo claramente.
Identifica todos los ingredientes visibles o muy probables, pero no inventes ingredientes invisibles.
Estima porciones y nutrientes aproximados para la comida completa.
Usa rangos razonables mentalmente, pero responde con un solo estimado central.
...
def solicitar_consulta(motivo, modalidad):
    ...
    return (
        ...
        f"**Motivo:** {motivo}\n\n"
        "Un nutriólogo podría revisar tu caso y darte orientación personalizada.\n\n"
        "Ganaste **15 XP**"
    )
...
def buscar_nutriologos(estado, ciudad):
    ...
    texto = (
        ...
        "Referencias públicas encontradas en sitios profesionales o directorios. "
...
with gr.Blocks(
    ...
    with gr.Tab("Nuti"):
        chat_nuti = gr.Chatbot(
            label="Nuti - guía de nutrición",
            type="messages",
            avatar_images=("assets/nutribot-user-chat-avatar.png", "assets/nutribot-robot-original-chat-avatar.png"),
            layout="bubble",
        )
        entrada_nuti = gr.Textbox(label="Escribe algo para Nuti...", interactive=False)
        boton_nuti = gr.Button("Enviar", variant="primary", interactive=False)

    with gr.Tab("Coach"):
        chat_coach = gr.Chatbot(
            label="Coach emocional",
            type="messages",
            avatar_images=("assets/nutribot-user-chat-avatar.png", "assets/nutribot-robot-original-chat-avatar.png"),
            layout="bubble",
        )
        entrada_coach = gr.Textbox(label="Cuéntame cómo te sientes hoy...", interactive=False)
    ...
    boton_registro_inicio_toggle.click(
        ...
        outputs=[dashboard_intro_panel, boton_registro_inicio_toggle, panel_registro_inicio, plan_inicio_panel, registro_inicio_abierto]
    )
    ...
    boton_nuti.click(
        conversar_nuti,
        inputs=[entrada_nuti, chat_nuti],
        outputs=[chat_nuti, entrada_nuti]
    )

    boton_coach.click(
        conversar_coach,
        inputs=[entrada_coach, chat_coach],
        outputs=[chat_coach, entrada_coach]
    )
